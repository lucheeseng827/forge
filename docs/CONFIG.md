...
| `--concurrency` | usize | `256` | Per-worker in-flight cap = the **ceiling** of the AIMD adaptive limiter. Set it to the engine's own knob (vLLM `--max-num-seqs`, SGLang `--max-running-requests`, llama-server `--parallel`). | Always match the engine; too high risks engine OOM (AIMD backs off but starts at the ceiling). |
